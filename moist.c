...
void insert(TreeNode* pTree, Data e){
	...
	if(e.val > pTree->value.val){
		if (isEmpty(pTree->pright) || (e.val<pTree->pright->value.val || (!(e.val<pTree->value.val) && (e.val==pTree->pleft->value.val && e.Num>pTree->pleft->value.Num)))){
			tamp= pTree->pright;
	...
	if(e.val < pTree->value.val){
		if (isEmpty(pTree->pleft) || (e.val>pTree->pleft->value.val || (!(e.val>pTree->value.val) && (e.val==pTree->pleft->value.val  && e.Num>pTree->pleft->value.Num)))){
			tamp= pTree->pleft;
   	...
   	/*else
    	{
		if (e.Num > pTree->value.Num){
			if (isEmpty(pTree->pright) || e.Num<pTree->pright->value.Num){
				tamp= pTree->pright;
				pTree->pright=createTree(e);
				pTree->pright->pright=tamp;
			}
    		...
    		else if (e.Num < pTree->value.Num){
        		if (isEmpty(pTree->pleft) || e.Num>pTree->pleft->value.Num){
				tamp= pTree->pleft;
				pTree->pleft=createTree(e);
				pTree->pleft->pleft=tamp;
			}
